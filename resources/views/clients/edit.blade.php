...
<div class="container mx-auto px-6 py-8">
    ...
    <form action="{{ route('clients.update', $client->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" class="w-full px-4 py-2 border rounded-lg" value="{{ $client->name }}" required>
        </div>
        <div class="mb-4">
            <label for="surname" class="block text-gray-700">Surname</label>
            <input type="text" name="surname" id="surname" class="w-full px-4 py-2 border rounded-lg" value="{{ $client->surname }}" required>
        </div>
        <div class="mb-4">
            <label for="identification_number" class="block text-gray-700">Identification Number</label>
            <input type="text" name="identification_number" id="identification_number" class="w-full px-4 py-2 border rounded-lg" value="{{ $client->identification_number }}" required>
        </div>
    ...
    <div id="data-objects-container">
        @if ($formattedData)
            @foreach ($formattedData as $objectIndex => $dataObject)
                <div class="data-object-group mb-4 p-4 border rounded-lg">
                    <div class="mb-2">
                        <label class="block text-gray-700">Object Name</label>
                        <input type="text" name="data[{{ $objectIndex }}][object_name]" placeholder="Object Name" class="w-full px-4 py-2 border rounded-lg" value="{{ $dataObject['object_name'] }}" required>
                    </div>
                    <div class="object-data-container">
                        @foreach ($dataObject['items'] as $itemIndex => $item)
                            <div class="object-data-item mb-2">
                                <div class="flex space-x-2">
                                    <input type="text" name="data[{{ $objectIndex }}][items][{{ $itemIndex }}][key]" placeholder="Key" class="w-1/4 px-4 py-2 border rounded-lg" value="{{ $item['key'] }}" required>
                                    <input type="text" name="data[{{ $objectIndex }}][items][{{ $itemIndex }}][value]" placeholder="Value" class="w-3/4 px-4 py-2 border rounded-lg" value="{{ $item['value'] }}" required>
                                </div>
                            </div>
                        @endforeach
                    </div>
    ...
    <button type="button" id="add-data-object" class="bg-green-600 text-white px-4 py-2 rounded-lg mt-2">Add Data Object</button>
        ...
        <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg">Update Client</button>
    </form>
</div>
